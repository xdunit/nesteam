...
- **for userapp app**:
  - 1.**/userapp/list/** — Данный маршрут предоставляет список **полной** информации о пользователях с использованием встроенной модели **User** фреймворка **Django**. Заметьте, что данная информация включает в себя зашифрованный пароль пользователей, обеспечивая конфиденциальность данных.
  - 2.**/userapp/user-router/** -- Данный маршрут предоставляет список информации о пользователях показывая ли те поля которые были указаны в нашем **сериализаторе** ( **'url', 'id', 'username', 'email', 'first_name'**) и при условий что данный пользователь является **активным: (filter(is_active=True))**
